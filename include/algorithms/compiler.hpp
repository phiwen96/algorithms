...
compiler::compiler (char const* source) : sc {source}
{
    advance ();
    expression ();
...
auto compiler::consume (token::type t, const char* msg) -> void
{
  if (prs.current.t == t)
  {
    advance ();
...
auto compiler::error_at_current(const char* msg) -> void
{
    error_at (&prs.current, msg);
}

auto compiler::error(const char* msg) -> void
{
    error_at (&prs.previous, msg);
}

auto compiler::error_at (token* tok, const char* msg) -> void
{
    if (prs.panic_mode)
    {
        return;
...
auto compiler::parse_precedence (precedence p) -> void
{
    advance();
...
auto compiler::emit_constants(int value) -> void
{
  emit_bytes (opcode::OP_CONSTANT, make_constant (value));
}

auto compiler::make_constant (int value) -> int
{
    consts += value;
...
auto compiler::emit_bytes (auto... b) -> void
{
    ((code += b), ...);
}
